...
image2D_ptr encadrer(image2D_ptr image_binarise,boite_englobante* bteEnglobante) {
    ...
    reconnaissance_objet(bteEnglobante);
...
void reconnaissance_objet(boite_englobante* bte){
    int lig_haut,lig_bas,col_haut,col_bas;
    lig_haut = bte->coordonnes[0];
    lig_bas = bte->coordonnes[1];
    col_haut = bte->coordonnes[2];
    col_bas = bte->coordonnes[3];
    int nb_pixel = 0;
    for(int i = lig_haut;i<=lig_bas;i++){
        for(int j = col_haut;j<=col_bas;j++){
            if(bte->image->image[i][j]==1) nb_pixel++;
        }
}
